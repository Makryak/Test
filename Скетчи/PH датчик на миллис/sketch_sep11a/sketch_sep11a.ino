...
#define Offset           -0.00                                                                // Компенсация смещения
                                                                 // Номер вывода светодиода, который является индикатором нормальной работы скетча
...
int pHArrayIndex=0;                                                                           // Индекс элемента массива pHArray значения которого требуется изменить
                                                                                              //
void setup(void){                                                                             //                                                                    // Конфигурируем вывод светодиода как выход
    lcd.init();
    lcd.backlight();
    Serial.begin(9600);                                                                       // Инициируем передачу данных в монитор последовательного порта на скорости 9600 бит/с
...
//  Выводим показания в монитор:                                                              //
    if(millis() - printTime > printInterval){                                                 // Выполняем код в теле оператора if через каждые printInterval мс
        lcd.print("Voltage:");                                                             // Выводим надпись "Voltage:"
        lcd.print(voltage,2);                                                              // Выводим среднее напряжение
        lcd.print("    pH value: ");                                                       // Выводим надпись "    pH value: "
        lcd.println(pHValue,2);                                                            // Выводим значение pH
